...
/**
* Poppin'Party Live2D 桌宠 —— host 半区路由。
* 同源静态路由 /pp-assets/* 暴露插件 assets 目录（vendor 运行时脚本、
...
const PET_ASSET_PREFIX = "/pp-assets";
const MIME = {
...
/** 静态资源前缀路由：/pp-assets/<assets 内相对路径>。 */
function makeAssetRoute(packageRoot) {
	return {
		...
		handler: (req, res) => {
			...
			const abs = assetPath(packageRoot, urlPath.startsWith("/pp-assets") ? urlPath.slice(10) : urlPath);
			if (abs === null) {
				res.writeHead(400);
				res.end();
				return;
...
const name = "dsh-live2d-poppinparty";
/** 注册静态资源路由；插件卸载时自动清理。 */
function apply(ctx) {
	const packageRoot = packageRootOf(import.meta.url);
	ctx.inject(["webServer"], (httpCtx) => {
		httpCtx.effect(() => httpCtx.webServer.register(makeAssetRoute(packageRoot)), "live2d-poppinparty: routes");
	});
}
//#endregion
